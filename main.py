...
class ShoppingCart:
    ...
    def remove_item(self, index):
        """Removes one unit of the item at given index"""
    ...
    def delete_item(self, index):
        """Deletes the entire item entry at given index"""
        if 0 <= index < len(self.items):
            product = list(self.items.keys())[index]
            del self.items[product]

    def update_item_quantity(self, index, new_quantity):
        """Updates the quantity of the item at given index"""
        if 0 <= index < len(self.items):
            product = list(self.items.keys())[index]
            if new_quantity <= 0:
                del self.items[product]
...
class POSApp(QWidget):
    def __init__(self):
        ...
        self.receipt_history_window = None  # To keep reference
